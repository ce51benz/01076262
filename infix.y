...
%{
  ...
  stack infixst; /*Stack for this parser.*/

  /*Error flag use to indicate that whether to show calculation result or not(It went 		right or went wrong).*/
  int errflag;
  ...
  */     
  ...
  /*Special variable which separate from typical array of long long variable (r[12])
	this variable keep address to top of stack.*/
  long long *rtop;
%}
...
line:
  '\n'
| exp '\n'  
  /*The action for this rules to check that if expression is enter
	if expression not cause an error,show caculation result
	otherwise reset flag to 0.*/
	    { if(!errflag){
		printf ("\t%lld\n", $1);r[RACC]=$1;
		}
	      else errflag = 0;
...
/*Action in COPY reg TO reg is described follows
	...
	check that is semantic value of 2nd component 
	is RTOP (which referencing to $top)
	check that whether $top keep something which 
	is not NULL? If yes COPY it from $top to register that 4th component is 	  referencing to,otherwise show error
	if register reference in 2nd component is others,
	COPY from it to register that 4th component is referencing to normally.*/
| COPY reg TO reg '\n' { if($4 == RTOP)
...
/*Action in PUSH reg TO reg is described follows
	...
	push value that $top (via dereferencing) keep to stack
	else show error
...
/*Action in PUSH reg TO reg is described follows
	...
	if no,check that stack size is not 0 via check value that $size keep
	if value is not zero, use push function to write value from top of stack
	target register,otherwise show 'empty stack' error.*/
| POP reg  '\n'{ if($2 == RTOP)
		yyerror("$top is READONLY!");
	     else if($2 == RSIZE)
		yyerror("$size is READONLY!");
...
that ours provides.*/
exp:
/*The first three rules is constant token,the action is assign semantic value of exp by use value of $1(or 1st component of match token.)*/
  NUMDEC             { $$ = $1;   }
| NUMBIN	     { $$ = $1;   }
| NUMHEX	     { $$ = $1;   }
...
array of long long and indexing to by use semantic value of reg as index number.
If no,check that is $top is not NULL.If it is not NULL,assign semantic value of exp
by use value that rtop reference to normally,otherwise show error and set errflag to 1.*/
| reg		     {
			if($1 != RTOP)
				$$ = r[$1];
			else if(rtop != NULL)
...
if stack is full,expand stack's maxsize by double(Remember to use malloc for 8*n
where n is maxsize value and 8 is size for 1 item that keep in stack.)
...
Update $top register value by use address of arr[size-1]*/
void push(stack *st,long long value){
...
set errflag to 0.For size and top properties in stack,both keep address of $top and $size register(NOT real value) finally use yyparse to start parsing.
*/
void main(){
errflag = 0;
rtop = NULL;
